...
This document outlines the technical feasibility, architectural design, and step-by-step implementation blueprint for enhancing the Maisone Sourcing ERP platform across all 13 stages, Role-Based Access Control (RBAC), Public Client Portal, Email Automations, Bulk Excel Import Workflows, and Zero-Touch Stage Automations.
...
- **Access Rules:**
  ...
  - 💼 **Agent Role:** Restricted/Masked view. Agents can view stage progression, upload tech packs/specs, manage client communications, and view _Buyer (Client) Costing_, but _Supplier (Factory) Costing_ and factory names are automatically masked or hidden.
...
- **Access Rules:**
  ...
  - Clients open this link without logging in to view real-time stage progression, dispatch notices, courier tracking IDs, and download approved quotations.
...
- **Features:**
  - **Auto-Sort Factories:** Intelligent matching algorithm scoring suppliers based on category (e.g. _Denim_, _Knitwear_, _Outerwear_), MOQ capacity, lead time, and quality ratings.
...
- **Features:**
  ...
  - 🎯 **IMPORTANT (Checkpoint 1):**
    - "Send Client Quotation" trigger sends formatted quote to client via tracking link and automated email notice.
...
- **Features:**
  - Client view & email notification include sample info, courier tracking ID (DHL/FedEx/etc.), and downloadable sample invoice.
...
- **Features:**
  ...
  - **Invoice Reminder:** One-click reminder button generating payment email notices.
...
- **Features:**
  - Log inspection date, outcome (`Passed`, `Failed`, `Rework`), defect notes, and upload inspection photos/reports manually.

---

### 🔹 Step 11 — Shipment Dispatch Notice

- **Feasibility:** 100% Feasible (Decoupled Email Trigger with Tracking ID)
- **Features:**
  - Send carrier tracking ID and ETA to the client via automated email independently without requiring Stage 12 (Bulk Invoice) to be finalized.
...
| :---------------------------- | :--------------------------------------------------- | :-------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **Stage 1: Enquiry Received** | Manual data entry of leads & sales agent assignment. | **Webhooks Auto-Ingest:** Ingests leads from website & email, pre-filling Enquiry ID, client details, and assigning agent by region.                      |
| **Stage 2: Sourcing**         | Manual searching & emailing factory directories.     | **Auto-Match Algorithm:** Scores top 3 factories by category & MOQ with 1-click RFQ dispatch.                                                             |
| **Stage 3: Costing**          | Calculating margins & drafting quote emails.         | **Auto-Margin Calculator:** Applies agency margin %, calculates FOB/CIF prices, generates PDF, and emails Checkpoint 1 link.                              |
...
| **Stage 5: Sample Invoice**   | Drafting sample invoices manually.                   | **1-Click Auto Invoice:** Pulls sample fee + courier cost from Stage 4 and generates `#SMP-INV-XXX` + emails tracking ID.                                 |
| **Stage 6: Sample Payment**   | Checking bank statements & typing received amounts.  | **Payment Balance Sync:** Updates balance outstanding, records payment, and unlocks Stage 7.                                                              |
| **Stage 7: Client Approval**  | Waiting for manual email replies & updating status.  | **Self-Service Client Approval:** Client clicks "Approve Sample" on tracking link $\rightarrow$ auto-logs sign-off timestamp and unlocks Stage 8.         |
...
| **Stage 9: Production**       | Calling/emailing factories for status updates.       | **Supplier 1-Click Update:** Factory receives a weekly email link to log completion % and upload progress photos.                                         |
| **Stage 10: Quality Check**   | Manual inspection logging.                           | **Manual QC Logger:** Inspector inputs inspection date, pass/fail status, defect notes, and uploads inspection photos/reports.                            |
| **Stage 11: Shipment**        | Manual tracking notifications & drafting emails.     | **Email Tracking Notice:** Entering Tracking ID & Carrier Name automatically emails tracking details & ETA to the client.                                 |
...
| **Stage 13: Bulk Payment**    | Monitoring bank accounts & drafting reminders.       | **Automated Overdue Engine:** T-3, T-0, and T+3 reminder emails trigger automatically if unpaid by settlement date.                                       |
...
### 2. Email Tracking ID & Carrier Dispatch Notification

- **Automation:** When a courier tracking ID and carrier name are saved in Stage 5 or Stage 11, the ERP automatically generates and emails a tracking summary notice with a direct link to the client.

### 3. Automated Agency Margin Calculator

- **Automation:** Calculates Buyer Costing automatically from Supplier Costing based on configured margin rules:
...
| **Email Tracking Notice**               |    100%     |           ❌ No            |        Yes (Email Notification Trigger)        |
