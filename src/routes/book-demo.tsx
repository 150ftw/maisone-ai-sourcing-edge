...
const STEPS = ["Your details", "Company", "Sourcing needs", "Review"];
...
function BookDemoPage() {
  ...
  const canNext =
    (step === 0 && form.fullName.trim() && form.workEmail.includes("@") && form.role.trim()) ||
    (step === 1 && form.company.trim()) ||
    step === 2 ||
    step === 3;
        ...
        <main className="relative z-10 mx-auto max-w-6xl px-6 pb-24 pt-6 grid lg:grid-cols-5 gap-10">
          ...
          <aside className="lg:col-span-2 space-y-8">
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-[10px] tracking-[0.25em] uppercase text-muted-foreground">
              <Sparkles className="size-3 text-electric" /> Private Demo
            </div>
            ...
            <div className="glass rounded-2xl p-5">
              <p className="text-[10px] uppercase tracking-widest text-muted-foreground">Trusted by</p>
              <div className="mt-3 flex flex-wrap gap-x-5 gap-y-2 opacity-70">
                {["MAISON KYŌ", "ATELIER LDN", "STUDIO MILANO", "HOUSE OF NY"].map((b) => (
                  <span key={b} className="font-serif text-sm tracking-[0.2em]">{b}</span>
                ))}
              </div>
            </div>
          </aside>
              ...
              {submitted ? (
                ...
                <>
                  ...
                  <div className="space-y-4">
                    ...
                    {step === 3 && (
                      <div className="rounded-2xl bg-background border border-border p-5 text-sm">
                        <ReviewRow k="Name" v={form.fullName} />
                        <ReviewRow k="Email" v={form.workEmail} />
                        <ReviewRow k="Role" v={form.role} />
                        <ReviewRow k="Company" v={form.company} />
                        <ReviewRow k="Size" v={form.companySize} />
                        <ReviewRow k="Region" v={form.region} />
                        <ReviewRow k="Category" v={form.category} />
                        <ReviewRow k="Volume" v={form.monthlyVolume} />
                        <ReviewRow k="Timeline" v={form.timeline} />
                        {form.message && <ReviewRow k="Notes" v={form.message} />}
                      </div>
                    )}
                  </div>
